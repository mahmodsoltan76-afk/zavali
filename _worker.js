...
const initializedD1Databases = new WeakSet();

async function d1Init(env) {
    const db = env.IOT_DB;
    if (!db || initializedD1Databases.has(db)) return;

    try {
        await db.prepare(
            "CREATE TABLE IF NOT EXISTS kv_store (key TEXT PRIMARY KEY, value TEXT)",
        ).run();
        initializedD1Databases.add(db);
    } catch (e) {
...
async function d1Get(env, key) {
    ...
    try {
        ...
        console.error(`D1 read failed for ${key}:`, e);
    }
    return null;
}
...
async function d1Put(env, key, value) {
    ...
    try {
        ...
        console.error(`D1 write failed for ${key}:`, e);
    }
}
        ...
        try {
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    const dashboardUrl = env.DASHBOARD_URL || 'https://raw.githubusercontent.com/mahmodsoltan76-afk/zavali/refs/heads/main/dashboard.html';
                    try {
                ...
                if (reqPath === routes.data) {
                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        ...
                        if (isValidUser) {
                            const subscriptionUrl = env.SUBSCRIPTION_URL || 'https://raw.githubusercontent.com/mahmodsoltan76-afk/zavali/refs/heads/main/subscription.html';
                            try {
    ...
    async scheduled(event, env, ctx) {
        try {
            ...
            if (sysConfig.autoUpdate && sysConfig.cfAccountId && sysConfig.cfApiToken && sysConfig.cfWorkerName) {
                const repo = (sysConfig.githubRepo || "mahmodsoltan76-afk/zavali")
...
async function loadSysConfig(env, ctx = null) {
    ...
    if (env.IOT_DB) {
        if (now - sysConfigCacheTime > CACHE_TTL_CONFIG) {
            try {
                ...
                if (migrateSlaveNodesToLinkedPanels(sysConfig)) {
                    const migrationPromise = cachedD1Put(
                        env,
                        "sys_config",
                        JSON.stringify(sysConfig),
                    ).catch((e) => {
                        console.error("sys_config migration write failed:", e);
                    });
                ...
                console.error("sys_config load failed:", e);
        ...
        if (now - sysUsageCacheTime > CACHE_TTL_USAGE) {
            try {
                ...
                console.error("sys_usage load failed:", e);
                sysUsageCache = { users: {} };
                sysUsageCacheTime = Date.now();
            }
        }
    ...
    if (now - backupIpCacheTime > CACHE_TTL_BACKUP_IP) {
        try {
            backupIpCache = env.IOT_DB
                ? await d1Get(env, "backup_ip")
                : null;
        } catch (e) {
            console.error("backup_ip load failed:", e);
        }
...
async function processTelemetryStream(env, ctx, wsRelayIdx) {
    ...
    startDataPipe(webSocket, env, ctx, wsRelayIdx).catch((err) => {
        console.error("startDataPipe failed:", err);
        try {
            webSocket.close();
        } catch (e) {}
    });
...
async function startDataPipe(webSocket, env, ctx, wsRelayIdx) {
    ...
    async function parseSensorData(bufferData, wsRelayIdx) {
        ...
        remoteSocket.readable
            ...
            .catch((err) => {
                console.error("Socket pipe failed:", err);
                try {
                    webSocket.close();
                } catch (e) {}
            });
